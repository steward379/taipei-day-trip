...
async function fetchData() {
  ...
  console.log(
    `Fetching data for page ${currentPage} (human ${currentPage + 1}  ) on ${
...
function setupSearchButtonListener() {
  ...
  searchButton.addEventListener("click", () => {
    ...
    const keywordInputSubmit = document.getElementById("search-input").value;
...
function createElements(attractions) {
  const mainElement = document.querySelector("main");
  const ulProfile =
    mainElement.querySelector(".photo-profile") || document.createElement("ul");
